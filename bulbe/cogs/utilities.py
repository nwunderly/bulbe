...
class Utilities(Cog):
    ...
    @commands.command()
    async def userinfo(
        self, ctx, user: Union[discord.Member, discord.User, FetchedUser] = None
    ):
        """Info about a user."""
        user = user if user else ctx.author
        embed = self.bot.Embed(
            description=user.id, color=user.color if user.color.value != 0 else None
        )
        ...
        embed.add_field(
            name="Shared Servers",
            value=f"{sum(g.get_member(user.id) is not None for g in self.bot.guilds)}",
            inline=False,
        )
        ...
        embed.add_field(
            name="Created", value=f"{user.created_at:%m/%d/%Y}", inline=False
        )

        if isinstance(user, discord.Member):
            embed.add_field(
                name="Joined",
                value=f"{getattr(user, 'joined_at', None):%m/%d/%Y}",
                inline=False,
            )
            ...
            perms = (
                ...
                if owner
                    ...
                    "administrator"
                    if admin
                    else (", ".join(permissions) if permissions else None)
                )
            )
    ...
    async def botinfo(self, ctx):
        ...
        embed.add_field(
            name="OS", value={"linux": "Ubuntu", "win32": "Windows"}[sys.platform]
        )
        ...
        embed.add_field(
            name="Source", value="[github](https://github.com/nwunderly/RevBots)"
        )
        embed.add_field(
            name="Add me!",
        ...
        embed.set_footer(
            text=f"bot by {self.bot._nwunder}", icon_url=self.bot._nwunder.avatar_url
        )
